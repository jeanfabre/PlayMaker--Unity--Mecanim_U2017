...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo2 : MonoBehaviour
	{
		...
		{
			if(CanMove && Input.GetKeyDown(SetMoveType)) type = TransformType.Move;

			else if(CanRotate && Input.GetKeyDown(SetRotateType)) type = TransformType.Rotate;

			else if(CanScale && Input.GetKeyDown(SetScaleType)) type = TransformType.Scale;

			if(CanSpace && Input.GetKeyDown(SetSpaceToggle))
			{
		...
		void GetTarget()
		{
			...
			if(selectedAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				{
					if (hitInfo.transform.CompareTag (tagFilter))
					{
